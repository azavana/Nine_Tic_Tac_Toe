...
7x7 TIC TAC TOE game. Implemented under Linux Ubuntu 16.04 LTS.
...
int main()
{
	...
	for (int count = 0; count < 81 ; count++)
	{
		if (turn == 1)
		{
			...
			{
				fprintf(stdout, "Square already filled by you own piece. Try another coordinates.\n\n");				
				goto label1;
			} 
			else if (attack_O(x,y,board)) // If player 1 attack player 2 piece; player 2 win.
			{
				fprintf(stdout, Red "FORBIDDEN MOVE. YOU TRY TO ATTACK PLAYER 2 PIECE AT POSITION. PLAYER 2 WIN !!!\n\n");
				fprintf(stdout, Reset);
				return EXIT_SUCCESS;
			}	
			board[x+1][y+1] = 'X';
		...
		else if (turn == 2)
		{
			...
			{
				fprintf(stdout, Red "FORBIDDEN MOVE. YOU TRY TO ATTACK PLAYER 1 PIECE AT POSITION. PLAYER 1 WIN !!!\n\n");
				fprintf(stdout, Reset);
				return EXIT_SUCCESS;
			}
			board[x+1][y+1] = 'O';
